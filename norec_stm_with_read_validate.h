...
#define TRY_LOCK(lock)                                                         \
  __sync_bool_compare_and_swap(&(lock), (lock) & ~1, lock | 1)
...
public:
  ...
  unsigned int tx_validate_read() {
    while (true) {
      ...
      if (_time == global_clock && snapshots == local_counters) {
        return _time;
      }
    }
  }
  ...
  int tx_read(int *address) {
    ...
    while (RV != global_clock || local_counters != snapshots) {
      RV = tx_validate_read();
      CFENCE;
      value = *address;
      CFENCE;
